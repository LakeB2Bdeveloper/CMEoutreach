...
const REPORT_URL = "email-deliverability-healthcare-marketers.html";
const UNLOCK_KEY = "cmeintent_deliverability_unlocked";

const HIGHLIGHTS = [
  { icon: "shield-check", title: "Email authentication", text: "How SPF, DKIM, and DMARC build sender credibility and keep accredited mail out of spam." },
  { icon: "database-zap", title: "Data hygiene at scale", text: "The governance practices that keep a healthcare contact database accurate as clinicians move." },
  { icon: "mail-check", title: "Validation before every send", text: "Reduce bounce rates and protect sender reputation by validating addresses pre-campaign." },
  { icon: "target", title: "Audience relevance", text: "Why specialty-level targeting lifts engagement and reinforces inbox placement over time." },
];
...
function GateForm({ onUnlock }) {
  ...
  return (
    <form onSubmit={submit} className="bg-white rounded-2xl border border-slate-200 shadow-xl p-7 sm:p-8">
      <h2 className="font-display font-extrabold text-2xl text-ink tracking-tight mb-1">Get the whitepaper</h2>
      ...
      <button
        ...
        Unlock the Whitepaper <Icon name="arrow-right" size={18} />
      </button>
...
function Unlocked() {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-xl p-8 text-center">
      ...
      <p className="text-sm mb-7" style={{ color: "rgb(6, 30, 63)" }}>Email Deliverability for Healthcare Marketers is ready. Open it below, then use the Download PDF button in the whitepaper to save your copy.</p>
      <div className="flex flex-col sm:flex-row gap-3 justify-center">
        <a href={REPORT_URL} className="px-6 py-3.5 bg-gradient-to-br from-gold2 to-gold text-ink font-bold rounded-lg shadow-lg hover:shadow-xl transition lift flex items-center justify-center gap-2 font-display">
          Open the Whitepaper <Icon name="arrow-right" size={18} />
        </a>
...
function ReportGatePage() {
  ...
  useEffect(() => {
    document.title = "Download · Email Deliverability for Healthcare Marketers | CMEIntent";
        ...
        <div className="max-w-7xl mx-auto relative z-10 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div>
            <div className="inline-flex items-center gap-2 bg-teal1/20 border border-teal3/30 text-teal3 text-[11px] font-bold tracking-wider uppercase px-4 py-1.5 rounded-full mb-6">
              <Icon name="mail-check" size={13} /> Whitepaper · Deliverability
            </div>
            <h1 className="font-display font-extrabold text-4xl sm:text-5xl text-white tracking-tight leading-[1.05] mb-5 text-balance">
              Email Deliverability for Healthcare Marketers
            </h1>
            <p className="text-lg text-white/75 leading-relaxed mb-8 max-w-xl">
              The hygiene, authentication, and list-validation practices that move accredited invitations from the spam folder back to the inbox.
            </p>
            <div className="flex flex-wrap gap-8">
              <Stat n="5 pages" l="Whitepaper" />
              <Stat n="4" l="Foundations" />
              <Stat n="7" l="Metrics to track" />
            </div>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <h2 className="font-display font-extrabold text-3xl sm:text-4xl text-ink tracking-tight text-balance">
            Four foundations, one playbook for the inbox
          </h2>
        </div>
