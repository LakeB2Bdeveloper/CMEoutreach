...
const REPORT_URL = "hcp-intent-data-guide.html";
const UNLOCK_KEY = "cmeintent_intent_data_unlocked";

const HIGHLIGHTS = [
  { icon: "activity", title: "What intent data is", text: "The behavioral signals that reveal what an HCP is researching today, beyond static profile attributes." },
  { icon: "git-branch", title: "The 4-stage workflow", text: "From signal collection to topic classification, intent scoring, and audience activation." },
  { icon: "gauge", title: "How intent is scored", text: "Why recency, frequency, and consistency separate a real pattern from isolated noise." },
  { icon: "trending-up", title: "Why it matters for CME", text: "How intent-informed targeting lifts engagement, attendance, and registrations." },
];
...
function GateForm({ onUnlock }) {
  ...
  return (
    <form onSubmit={submit} className="bg-white rounded-2xl border border-slate-200 shadow-xl p-7 sm:p-8">
      <h2 className="font-display font-extrabold text-2xl text-ink tracking-tight mb-1">Get the guide</h2>
      ...
      <button
        ...
        Unlock the Guide <Icon name="arrow-right" size={18} />
      </button>
...
function Unlocked() {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-xl p-8 text-center">
      ...
      <p className="text-sm mb-7" style={{ color: "rgb(6, 30, 63)" }}>The HCP Intent Data guide is ready. Open it below, then use the Download PDF button in the guide to save your copy.</p>
      <div className="flex flex-col sm:flex-row gap-3 justify-center">
        <a href={REPORT_URL} className="px-6 py-3.5 bg-gradient-to-br from-gold2 to-gold text-ink font-bold rounded-lg shadow-lg hover:shadow-xl transition lift flex items-center justify-center gap-2 font-display">
          Open the Guide <Icon name="arrow-right" size={18} />
        </a>
...
function ReportGatePage() {
  ...
  useEffect(() => {
    document.title = "Download · HCP Intent Data: A Healthcare Marketer's Guide | CMEIntent";
  ...
  return (
    <div className="bg-white text-ink font-sans antialiased selection:bg-teal2 selection:text-white">
      <Nav />
      <header className="relative bg-gradient-to-br from-[#03101F] via-[#0A1E44] to-[#1A2E6B] pt-32 pb-24 px-6 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div>
            <div className="inline-flex items-center gap-2 bg-teal1/20 border border-teal3/30 text-teal3 text-[11px] font-bold tracking-wider uppercase px-4 py-1.5 rounded-full mb-6">
              <Icon name="activity" size={13} /> Whitepaper · Intent Data
            </div>
            <h1 className="font-display font-extrabold text-4xl sm:text-[44px] text-white tracking-tight leading-[1.07] mb-5 text-balance">
              HCP Intent Data: A Healthcare Marketer's Guide to In-Market Audiences
            </h1>
            <p className="text-lg text-white/75 leading-relaxed mb-8 max-w-xl">
              How behavioral signals reveal what healthcare professionals are researching today, and how CME providers turn those signals into timely, relevant campaigns.
            </p>
            <div className="flex flex-wrap gap-8">
              <Stat n="5 pages" l="Guide" />
              <Stat n="4 stages" l="Intent workflow" />
              <Stat n="Who + What" l="Data combined" />
            </div>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <h2 className="font-display font-extrabold text-3xl sm:text-4xl text-ink tracking-tight text-balance">
            From behavioral signals to in-market audiences
          </h2>
        </div>
